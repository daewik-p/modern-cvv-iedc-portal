  import { ChevronDown, Users, Award, Star } from "lucide-react";
  import { type ExecomMember, execomMembers,mediaTeamMembers,createTeamMembers,orbitTeamMembers } from "@/data/execom";
  ...
  const teamYears = [
    {
      ...
      teams: {
        ...
        "Media Team":mediaTeamMembers,
        "Create Team": createTeamMembers,
        "Orbit Team": orbitTeamMembers,
        }
      }
  ];
  /**
  ...
  const ExecomSection = memo(({ title, members, priority = 'low' }: { 
    ...
    const nodalOfficers = members.filter(m => m.role.toLowerCase().includes("nodal"));
    const iphMembers = members.filter(m => m.role.toLowerCase().includes("chief") || m.role.toLowerCase().includes("executive"));
    const otherMembers = members.filter(m => !nodalOfficers.includes(m) && !iphMembers.includes(m));
    useEffect(() => {
  ...
  <div className="space-y-12">
    ...
    {otherMembers.length > 0 && (
      <MemberGrid members={otherMembers} />
    )}
